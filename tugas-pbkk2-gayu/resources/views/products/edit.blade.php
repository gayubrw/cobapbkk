...
<div class="container mx-auto px-4">
    ...
    <form action="{{ route('products.update', $product->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="name" value="{{ $product->name }}" class="w-full px-3 py-2" required>
        </div>
        <div class="mb-4">
            <label for="price" class="block text-white">Harga:</label>
            <input type="number" name="price" value="{{ $product->price }}" class="w-full px-3 py-2" required>
        </div>
        <div class="mb-4">
            <label for="stock" class="block text-white">Stok:</label>
            <input type="number" name="stock" value="{{ $product->stock }}" class="w-full px-3 py-2" required>
        </div>
        <div class="mb-4">
            <label for="status" class="block text-white">Status:</label>
            <select name="status" class="w-full px-3 py-2" required>
        ...
        <div class="mb-4">
            ...
            <input type="url" name="image_url" value="{{ $product->image_url }}" class="w-full px-3 py-2">
        </div>
